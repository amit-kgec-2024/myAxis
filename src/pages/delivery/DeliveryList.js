  ...
  const dataListGet = async (page = 1) => {
    try {
      const response = await fetch(
        environment.apiUrl + `delivery/list?page=${page}`
      );
      if (!response.ok) {
  ...
  return (
    <Layout>
      <Container className="flex flex-row items-center justify-between">
        <H1>Delivery Partners List</H1>
        <AddButton onClick={() => router("/delivery-add")}>
                ...
                <tr key={index} className="border-b-2">
                  ...
                  <td className="p-3">
                    <span className="flex flex-row justify-center gap-4">
                      <button
                        onClick={() => {
                          sessionStorage.setItem("deliveryPartnerId", item.id);
                          router("/delivery-edit");
                        }}
                      >
