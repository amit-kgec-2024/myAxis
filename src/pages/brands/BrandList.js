...
const BrandList = () => {
  ...
  const dataListGet = async (page = 1) => {
    ...
    try {
      const response = await fetch(
        environment.apiUrl + `brand/list?page=${page}`
      );
      if (!response.ok) {
  ...
  const dataListGetDeactive = async (page = 1) => {
    ...
    try {
      const responsedeactive = await fetch(
        environment.apiUrl + `brand/list/deactive?page=${page}`
      );
  ...
  const handleToggle = async (id) => {
    try {
      const response = await fetch(environment.apiUrl + `brand/status/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <Layout>
      <Container className="flex flex-row items-center justify-between">
        <H1>Brands List</H1>
        <AddButton onClick={() => router("/brand-add")}>Add Brands</AddButton>
      </Container>
                ...
                <tr key={index} className="border-b-2">
                  ...
                  <td className="p-3">
                    <span className="flex flex-row justify-center gap-4">
                      <button
                        onClick={() => {
                          sessionStorage.setItem("brandId", item.id);
                          router("/brand-edit");
                        }}
                      >
