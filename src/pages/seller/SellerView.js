...
const SellerView = () => {
  ...
  const handelDelete = async (e) => {
    ...
    const isConfirmed = window.confirm(
      "Are you sure you want to Permanent delete this Sellers?"
    );
    if (!isConfirmed) return;
    try {
      const response = await fetch(
        environment.apiUrl + `seller/delete/${sellerId}`,
        {
          method: "DELETE",
          headers: {
      ...
      if (response.ok) {
        toast.success("Seller Permanent Delete Successfully");
        navigate("/seller-list");
      } else {
  ...
  const handelStatus = async (e) => {
    ...
    try {
      const response = await fetch(
        environment.apiUrl + `seller/status/${sellerId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        fetchSellerData(sellerId);
        toast.success(
          `Seller ${isActive === true ? "DeActive" : "Active"} Successfully`
        );
      } else {
        console.error("Failed to delete bank from the server");
      }
