...
const ParentcategoryList = () => {
  ...
  const dataListGet = async (page = 1) => {
    ...
    try {
      const response = await fetch(
        environment.apiUrl + `parentcategory/list?page=${page}`
      );
      if (!response.ok) {
  ...
  const dataListGetDeactive = async (page = 1) => {
    ...
    try {
      const responsedeactive = await fetch(
        environment.apiUrl + `parentcategory/list/deactive?page=${page}`
      );
  ...
  const deleteParentCategory = async (id) => {
    if (
      window.confirm("Are you sure you want to delete this parent category?")
    ) {
      try {
        const response = await fetch(
          environment.apiUrl + `parentcategory/delete/${id}`,
          {
            method: "DELETE",
            headers: {
  ...
  const handleToggle = async (id) => {
    try {
      const response = await fetch(
        environment.apiUrl + `parentcategory/status/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const filteredList = list.filter((item) =>
    // active
    (item.parentCategoryName?.toLowerCase() || "").includes(
  ...
  return (
    <Layout>
      <Container className="flex flex-row items-center justify-between">
        ...
        <AddButton onClick={() => router("/parentcategory-add")}>
          Add Parent Category
        </AddButton>
      </Container>
            ...
            <tbody>
              {filteredList.map((item, index) => (
                <tr key={index} className="border-b-2">
                  ...
                  <td className="p-3">{item.categoryCount}</td>
                  <td className="p-3">{item.brandCount}</td>
                  ...
                  <td className="p-3">
                    <span className="flex flex-row justify-center gap-4">
                      <button
                        onClick={() => {
                          sessionStorage.setItem("parentcategoryId", item.id);
                          router("/parentcategory-edit");
                        }}
                      >
                        <MdEditSquare className="text-2xl" />
                      </button>
                      <button onClick={() => deleteParentCategory(item.id)}>
