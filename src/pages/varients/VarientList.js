...
const VarientList = () => {
  ...
  const dataListGet = async (page = 1) => {
    ...
    try {
      const response = await fetch(
        environment.apiUrl + `varient/list?page=${page}`
      );
      if (!response.ok) {
  ...
  const dataListGetDeactive = async (page = 1) => {
    ...
    try {
      const responsedeactive = await fetch(
        environment.apiUrl + `varient/list/deactive?page=${page}`
      );
  ...
  const deleteVarient = async (id) => {
    if (window.confirm("Are you sure you want to delete this varient?")) {
      try {
        const response = await fetch(
          environment.apiUrl + `varient/delete/${id}`,
          {
            method: "DELETE",
            headers: {
  ...
  const handleToggle = async (id) => {
    try {
      const response = await fetch(
        environment.apiUrl + `varient/status/${id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const filteredList = list.filter((item) =>
    // active
    (item.varientName?.toLowerCase() || "").includes(searchTerm.toLowerCase())
  );
  const filteredListDeactive = listDeactive.filter((item) =>
    // deactive
    (item.varientName?.toLowerCase() || "").includes(
  ...
  return (
    <Layout>
      <Container className="flex flex-row items-center justify-between">
        <H1>Varient List</H1>
        <AddButton onClick={() => router("/varient-add")}>
          Add Varient
        </AddButton>
      </Container>
                ...
                <tr key={index} className="border-b-2">
                  ...
                  <td className="p-3">
                    <span className="flex flex-row justify-center gap-4">
                      <button
                        onClick={() => {
                          sessionStorage.setItem("varientId", item.id);
                          router("/varient-edit");
                        }}
                      >
                        <MdEditSquare className="text-2xl" />
                      </button>
                      <button onClick={() => deleteVarient(item.id)}>
